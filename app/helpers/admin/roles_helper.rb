module Admin::RolesHelper
  ...
  def build_action_list_item(key, node)
    if node.has_key?(:children)
      ...
      li = %{<li class="checkbox-group"><input type="checkbox"  class="parent" id="action_#{node[:id]}" name="action_#{node[:id]}" /><label class='parent active'><span class='arrow'></span>#{key.capitalize}</label><ul class="sub">#{sub}</ul></li>}
    else
